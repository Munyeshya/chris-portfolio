  ...
  useEffect(() => {
    ...
    api('/content').then(({ team, partners, work }) => {
      ...
      setContent({
        ...
        work: work?.length ? work.map(item => ({ title: item.title, url: item.external_url, image: item.image_url, categories: item.categories, width: item.width, height: item.height })) : fallbackWork,
      })
    }).catch(() => undefined)
    return () => { active = false }
  }, [])
