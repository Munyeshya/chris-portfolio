...
function Brand({ dark = false }) {
  return (
    <a href="#home" className="brand" aria-label="Lions Entertainment — home">
      <span className="logo-crop">
        <img src={`/brand/lions-ent-${dark ? 'black' : 'white'}.png`} alt="Lions Entertainment" width="5404" height="3414" />
      </span>
    </a>
  )
}
    ...
    <>
      ...
      <main id="main" tabIndex={-1}>
        <section className="hero" id="home" data-section="02-hero" aria-labelledby="hero-title">
          ...
          <div className="hero-content section-shell">
            ...
            <div className="hero-copy">
              <p>Lions Entertainment combines professional AV production, creative media, and digital technology to help businesses, organizations, and events communicate, connect, and create memorable experiences.</p>
        ...
        <section className="about section-shell" id="about" data-section="03-about" aria-labelledby="about-title">
          ...
          <div className="about-copy"><p>Lions Entertainment is a creative production and technology company with over 5 years of experience delivering professional AV production, media, event, and digital solutions.</p><p>We combine creativity and technology to provide photography, videography, livestreaming, LED displays, web design, and web application development for businesses, organizations, and events.</p></div>
        ...
        <section className="events section-shell" id="event-solutions" data-section="05-event-solutions" aria-labelledby="events-title">
          ...
          <div className="events-content"><p>Lions Entertainment supports clients with both event planning and event management solutions, helping organize smooth, professional, and well-coordinated events from preparation to event day.</p><ul className="event-list">{eventSolutions.map((item, index) => <li key={item}><span aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>{item}</li>)}</ul><p className="events-closing">Whether it is a conference, corporate event, exhibition, launch, or private event, we help manage the details so the entire event runs smoothly.</p></div>
        ...
        <section className="team section-shell" id="team" data-section="08-team" aria-labelledby="team-title">
          ...
          <div className="team-heading"><SectionLabel>THE PEOPLE BEHIND THE WORK</SectionLabel><h2 id="team-title">Our <span>Team</span></h2><h3>Meet the creative and technical team behind Lions Entertainment.</h3><p>Our team brings together experience in audiovisual production, photography, videography, livestreaming, event production, design, and technology to deliver reliable solutions from concept to execution.</p></div>
          <div className="team-grid">{team.map((member, index) => <article className="team-card" key={member.name || index}><div className="team-photo">{member.photo ? <img src={member.photo} alt={member.name} loading="lazy" /> : <><span className="portrait-placeholder" aria-hidden="true"><i /><b /></span><span className="photo-label">PHOTO</span></>}</div><h3>{member.name || 'Name'}</h3>{member.role && <p>{member.role}</p>}</article>)}</div>
        </section>

        <section className="contact section-shell" id="contact" data-section="09-contact" aria-labelledby="contact-title">
          <div className="contact-heading"><SectionLabel light>Contact Us</SectionLabel><h2 id="contact-title">Let's Work<br /><span>Together.</span></h2><p>Planning an event, production, or digital project? Talk to Lions Entertainment and let's turn your idea into a professional solution.</p></div>
          <div className="contact-content">
            <dl className="contact-details">
              ...
              <div><dt>Follow Us</dt><dd className="social-icon-links"><a href={company.socials.Instagram} target="_blank" rel="noreferrer" aria-label="Lions Entertainment on Instagram" title="Instagram"><FaInstagram aria-hidden="true" /></a><a href={company.socials.YouTube} target="_blank" rel="noreferrer" aria-label="Lions Entertainment on YouTube" title="YouTube"><FaYoutube aria-hidden="true" /></a></dd></div>
