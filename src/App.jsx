...
function Brand({ dark = false }) {
  return (
    <a href="#home" className="brand" aria-label="Lions Plus — home">
      <span className="logo-crop">
        <img src={`/brand/lions-plus-${dark ? 'black' : 'white'}.png`} alt="Lions Plus" width="5404" height="3414" />
      </span>
    </a>
  )
}
    ...
    <>
      ...
      <main id="main" tabIndex={-1}>
        ...
        <section className="events section-shell" id="event-solutions" data-section="05-event-solutions" aria-labelledby="events-title">
          ...
          <div className="events-content"><p>Lions Plus supports clients with both event planning and event management solutions, helping organize smooth, professional, and well-coordinated events from preparation to event day.</p><ul className="event-list">{eventSolutions.map((item, index) => <li key={item}><span aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>{item}</li>)}</ul><p className="events-closing">Whether it is a conference, corporate event, exhibition, launch, or private event, we help manage the details so the entire event runs smoothly.</p></div>
        ...
        <section className="contact section-shell" id="contact" data-section="09-contact" aria-labelledby="contact-title">
          <div className="contact-heading"><SectionLabel light>Contact Us</SectionLabel><h2 id="contact-title">Let's Work<br /><span>Together.</span></h2><p>Planning an event, production, or digital project? Talk to Lions Plus and let's turn your idea into a professional solution.</p></div>
          <div className="contact-content">
            <dl className="contact-details">
              ...
              <div><dt>Follow Us</dt><dd className="social-icon-links"><a href={company.socials.Instagram} target="_blank" rel="noreferrer" aria-label="Lions Plus on Instagram" title="Instagram"><FaInstagram aria-hidden="true" /></a><a href={company.socials.YouTube} target="_blank" rel="noreferrer" aria-label="Lions Plus on YouTube" title="YouTube"><FaYoutube aria-hidden="true" /></a></dd></div>
